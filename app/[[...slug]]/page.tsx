...
const ABOUTUS_URL = RAW_BASE ? `${RAW_BASE}/aboutus` : '';
const SEO_LIST_URL = RAW_BASE
  ? `${RAW_BASE}/seo`
  : 'http://localhost:7000/landing/seo';
...
const CONTACT_URL =
  ...
  try {
    const res = await fetchWithISR(ABOUTUS_URL, {
...
/** Fetch Office Information data with ISR */
export async function fetchOfficeInformation() {
  try {
    ...
    console.error("Failed to fetch Office Information:", err);
    return null;
  }
}
  ...
  const faqs = [
    {
      ...
      answer:
        'Our minimum order quantity varies by fabric type, typically starting from 500 meters for standard fabrics and 1000 meters for custom specifications.',
    },
    {
      question: 'Do you provide fabric samples before placing bulk orders?',
      answer:
        'Yes, we provide free fabric samples for all our products. Sample orders are processed within 3-5 business days and shipped worldwide.',
    },
    {
